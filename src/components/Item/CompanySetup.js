...
if (user?.email) {
  setProfile(prev => {
    if (prev.CreatedBy) return prev;
    return { ...prev, CreatedBy: user.email };
  });
}
  ...
  pushToast("Company profile loaded", "success");
    ...
    if (!noProfileToastShown.current) {
      ...
      pushToast("No company profile found. Fill and save one.", "warning");
        ...
        if (msg.action === "SaveCompanyProfile") {

  // 🔴 HANDLE VALIDATION FIRST (NEW)
  if (msg.validation && !msg.validation.isValid) {
    setErrors(msg.validation.errors || {});

    // Optional: show modal
    const message = Object.values(msg.validation.errors || {}).join("\n");
  ...
  if (msg.success) {
    ...
    setModal({
      show: true,
      message: msg.message || "Company profile saved successfully",
    ...
    // ❌ NON-VALIDATION ERROR (DB / exception)
...
<div style={{
  ...
  ⚠️ Only one company profile can be created. After saving, you can only update it.
</div>
...
<div className="company-form-group">
  ...
  />
      ...
      <div className="company-actions">
        <button className="company-btn" onClick={handleSave}>
