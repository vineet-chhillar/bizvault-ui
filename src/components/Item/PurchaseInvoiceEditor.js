...
if (!supplierDraft.State.trim()) {
  errors.State = "State is required for GST calculation";
}
...
const toggleItemDetails = (i) => {
 ...
 const updateLine = (i, key, val) => {
  ...
  delete updated[`${key}_${i}`];   // remove error for this field
...
useEffect(() => {
  setLines(prev => [...prev]); // trigger recalculation
}, [supplierDraft.State]);
  // ========= RECALC TOTALS =========
 ...
 const isInterState = () => { 
  ...
  const buyer =
    supplierInfo?.State?.toLowerCase().trim() ||
    supplierDraft?.State?.toLowerCase().trim();   // ✅ supports draft
...
if (!invoiceNo.trim()) {
  errors["InvoiceNo"] = "Supplier Invoice No is required";
}

if (invoiceNo.length > 50) {
  errors["InvoiceNo"] = "Invoice No is too long";
}

// 🔴 CREDIT SUPPLIER VALIDATION
if (paymentMode === "Credit" && !supplierId) {
  const isValid = validateSupplierDraft();
  if (!isValid) {
    errors["SupplierDraft"] = "Invalid supplier details";
  }
}
...
lines.forEach((l, i) => {
  ...
  if (!l.ItemId || !l.ItemName.trim()) {
    errors[`ItemName_${i}`] = `Row ${i + 1}: Item is required`;
  }

  if (!qty || qty <= 0) {
    errors[`Qty_${i}`] = `Row ${i + 1}: Quantity must be greater than 0`;
  }

  if (!rate || rate <= 0) {
    errors[`Rate_${i}`] = `Row ${i + 1}: Purchase Rate must be greater than 0`;
  }
});
...
// ---------------- STORE PAYLOAD ----------------
  pendingSavePayloadRef.current = {
    ...
    SupplierState: supplierInfo?.State || supplierDraft?.State || "",
...
//}
  ...
  // ✅ Set default state in supplier draft
  if (msg.profile?.State) {
    setSupplierDraft(prev => ({
      ...prev,
      State: msg.profile.State
    }));
  }
}
...
if (msg.action === "CheckDuplicatePurchaseInvoiceResponse") {
  ...
  if (msg.exists) {
    setModal({
  show: true,
  message: "This supplier invoice number already exists.",
  type: "error"
});
...
return;
 ...
 if (msg.action === "SavePurchaseInvoiceResponse") {
  ...
  setModal({
    ...
    message:
      `Purchase Invoice ${msg.invoiceNo} saved successfully`,
...
<div className="customer-section">
  ...
  // 🔴 CLEAR SUPPLIER VALIDATION ERROR
  setValidationErrors(prev => {
    const updated = { ...prev };
...
<select
  ...
  onChange={e =>
    setSupplierDraft(d => ({ ...d, State: e.target.value }))
  }
>
...
{supplierErrors.State && (
  <div className="field-error">{supplierErrors.State}</div>
)}
...
{/* 🔴 INLINE VALIDATION (LIKE EDIT PAGE) */}
  ...
  <div className="suggestions-box" style={{ width: "320px" }}>
    {itemList
      .filter(it => {
        ...
        if (!l.ItemName?.trim()) return true;
