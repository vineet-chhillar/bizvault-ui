...
if (paymentMode === "Credit" && !data.CustomerId) {
  ...
  prev.filter(msg => !msg.includes(`Line ${i + 1}:`))
);
    setLines(prev => {
      const copy = [...prev];
...
useEffect(() => {
  ...
  if (!invoiceId || Number(invoiceId) <= 0) {
    setModal({
  show: true,
  message: "Select a sales invoice first.",
  type: "error"
});
return;
  }
...
useEffect(() => {
  ...
  if (!data.CustomerId || !data.CustomerName) {
    ...
    setModal({
      show: true,
      message: "Customer linked to this invoice no longer exists.",
      type: "error"
    });
  ...
  setModal({
    ...
    message:
      `Sales return saved. ReturnId = ${msg.returnId}`,
